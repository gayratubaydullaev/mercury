...
export default function EditProductPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!imageUrls.length) {
      toast.error('Kamida bitta rasm boʻlishi kerak');
      return;
    }
    const options: Record<string, string[]> = {};
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Masalan: Erkaklar koʻylagi, M" className="mt-1" required />
            </div>
            <div>
              <label className="text-sm font-medium">Tavsif *</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Mahsulot haqida qisqacha: material, oʻlchamlar" className="mt-1 w-full min-h-[120px] rounded-md border border-input bg-background px-3 py-2 text-sm" required />
            </div>
            <div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input type="text" inputMode="numeric" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="Masalan 99900" className="mt-1" />
              </div>
              <div>
                ...
                <Input type="text" inputMode="numeric" value={comparePrice} onChange={(e) => setComparePrice(e.target.value)} placeholder="Masalan 150000" className="mt-1" />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <Input value={sku} onChange={(e) => setSku(e.target.value)} placeholder="Masalan: KOYL-001" className="mt-1" />
              </div>
            </div>
            <div>
            ...
            {optionsRows.some((r) => r.name.trim() && r.values.split(',').map((v) => v.trim()).filter(Boolean).length > 0) && (
              <div>
                ...
                <p className="text-xs text-muted-foreground mb-2">Har bir kombinatsiya uchun qoldiq va ixtiyoriy rasm.</p>
