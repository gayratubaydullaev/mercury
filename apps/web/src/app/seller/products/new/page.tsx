...
export default function NewProductPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!categoryId) {
      alert('Kategoriyani tanlang (faqat ostkategoriya)');
      return;
    }
    setLoading(true);
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="text-2xl font-bold">Yangi tovar</h1>
      ...
      <form onSubmit={submit} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            {optionsRows.some((r) => r.name.trim() && r.values.split(',').map((v) => v.trim()).filter(Boolean).length > 0) && (
              <div>
                ...
                <p className="text-xs text-muted-foreground mb-2">Har bir kombinatsiya uchun qoldiq va ixtiyoriy rasm URL.</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">Cloudinary sozlangan boʻlishi kerak. Aks holda yuklash ishlamaydi.</p>
          </CardContent>
        </Card>
