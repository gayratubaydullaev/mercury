...
export default function MyOrdersPage() {
  ...
  if (!token) return <div className="max-w-2xl mx-auto px-4 py-8"><p className="text-muted-foreground">Kirish kerak.</p><Button asChild><Link href="/auth/login?next=/orders">Kirish</Link></Button></div>;
  if (forbidden) return <div className="max-w-2xl mx-auto px-4 py-8"><p className="text-muted-foreground mb-4">Buyurtmalar faqat xaridorlar uchun. Sotuvchi boʻlsangiz, doʻkon buyurtmalari Sotuvchi kabinetida.</p><Button asChild variant="outline"><Link href="/account">← Profil</Link></Button></div>;
  if (!data) return <div className="max-w-2xl mx-auto px-4"><Skeleton className="h-48 w-full rounded-xl" /></div>;
